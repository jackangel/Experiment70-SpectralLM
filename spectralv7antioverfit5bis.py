...
# ==========================================
# Set these to False to disable specific recommendations.
# The Proxy classes will handle shape mismatches automatically.
ABLATION_CONFIG = {
    "use_adaptive_segmentation": True,  # Rec #1
    "use_hyperbolic_vq": True,          # Rec #10 (False = Euclidean/Standard VQ)
    "use_disentangled_vq": True,        # Rec #4 (False = Single Stream VQ)
    "use_graph_processor": True,        # Rec #6 & #2
    "use_predictive_coding": True,      # Rec #5
    "use_contrastive_loss": False,       # Rec #3
    "use_causal_loss": False,            # Rec #8
    "use_meta_learner": True,           # Rec #7
    "use_symbolic_rules": True          # Rec #9
}
...
class ParquetDataLoader:
    def __init__(self, folder_path, text_column, tokenizer, chunk_size=10000):
        ...
        if not self.parquet_files: 
            # Allow running without files for testing
...
# -----------------------------
# PROXY CLASSES (The "Shape Correctors")
# -----------------------------
class ProxySegmenter(nn.Module):
    """
    Replaces AdaptiveSegmenter. 
    Instead of dynamic segments, it treats every token as a segment (1:1).
    Maintains the (Segments, Boundaries, Logits) return signature.
    """
    ...
    def forward(self, x):
        B, T, D = x.shape
        # Segments are just the input tokens
        segments = x 
        # Boundaries: Every token is a boundary
        boundaries = [torch.arange(T, device=x.device) for _ in range(B)]
        # Dummy logits
        boundary_logits = torch.zeros(B, T, 2, device=x.device)
        return segments, boundaries, boundary_logits

class ProxyVQ(nn.Module):
    """
    Replaces Hyperbolic/Disentangled VQ.
    Performs standard Euclidean VQ or Identity to maintain shapes.
    """
    ...
    def forward(self, inputs):
        ...
        flat_input = inputs.reshape(-1, self.embedding.weight.shape[1])
        dists = torch.cdist(flat_input, self.embedding.weight)
        encoding_indices = torch.argmin(dists, dim=1).unsqueeze(1)
        
        encodings = torch.zeros(encoding_indices.shape[0], self.embedding.weight.shape[0], device=inputs.device)
...
class ProxyDisentangledVQ(nn.Module):
    """
    Replaces DisentangledVQ.
    Uses a single stream VQ but returns dummy style/content splits to satisfy
    the 5-tuple return signature of the original module.
    """
    def __init__(self, d_model, num_buckets):
        super().__init__()
        self.vq = ProxyVQ(num_buckets, d_model)
        
    def forward(self, x):
        # Single stream quantization
        quantized, loss, indices = self.vq(x)
        
        # Return signature: combined, total_loss, content_idx, style_idx, disentangle_loss
        # We reuse 'indices' for both content and style to prevent crashes downstream
        return quantized, loss, indices, indices, torch.tensor(0.0, device=x.device)

class ProxyGraphProcessor(nn.Module):
    """Replaces SemanticGraphProcessor. Identity pass-through."""
    def __init__(self):
        super().__init__()
...
class ProxyPredictiveLayer(nn.Module):
    """Replaces PredictiveCodingLayer. Identity pass-through."""
    def __init__(self):
        super().__init__()
        
    def forward(self, bottom_up_input, top_down_prediction=None):
        # Returns: corrected, next_prediction, prediction_error
        # We return input as corrected, input as prediction, 0 error
        return bottom_up_input, bottom_up_input, torch.tensor(0.0, device=bottom_up_input.device)
...
# -----------------------------
# 4. Disentangled Content/Style VQ (Recommendation #4)
# -----------------------------
class DisentangledVQ(nn.Module):
    """Separates what is said (content) from how it's said (style)"""
    def __init__(self, d_model, num_content_buckets, num_style_buckets, use_hyperbolic=True):
        super().__init__()
        ...
        if use_hyperbolic:
            ...
            self.content_vq = ProxyVQ(num_content_buckets, self.content_dim)
            self.style_vq = ProxyVQ(num_style_buckets, self.style_dim)
...
# -----------------------------
# 6. Graph Neural Network (Recommendation #6)
# -----------------------------
...
# -----------------------------
# 7. Predictive Coding Layer (Recommendation #5)
# -----------------------------
class PredictiveCodingLayer(nn.Module):
    ...
    def __init__(self, d_model, n_frequencies):
        ...
        self.error_weight = nn.Parameter(torch.tensor(0.1))
...
# -----------------------------
# 9. Contrastive Predictive Coding (Recommendation #3)
# -----------------------------
...
# -----------------------------
# 12. Causal Intervention Module (Recommendation #8)
# -----------------------------
class CausalInterventionLoss(nn.Module):
    ...
    def forward(self, model, x, bucket_embeddings):
        ...
        current_positions = bucket_embeddings[:, :-1]
        
        normal_predicted_next = self.causal_proj(current_positions)
        
        noise = torch.randn_like(current_positions) * self.intervention_noise
        intervened_positions = current_positions + noise
        
        intervened_predicted_next = self.causal_proj(intervened_positions)
...
class CognitiveSpectralLM(nn.Module):
    def __init__(self, vocab_size, ablation_config, d_model=384, n_layers=6, n_frequencies=128, 
        ...
        if self.config["use_adaptive_segmentation"]:
            self.segmenter = AdaptiveSegmenter(d_model, max_segment_len=32)
        else:
            self.segmenter = ProxySegmenter(d_model)

        # --- ABLATION: Disentangled VQ ---
        if self.config["use_disentangled_vq"]:
            self.disentangled_vq = DisentangledVQ(
                d_model, num_content_buckets, num_style_buckets, 
                use_hyperbolic=self.config["use_hyperbolic_vq"]
            )
        else:
            # Fallback: Single stream VQ (replicated for content/style to match shape)
            self.disentangled_vq = ProxyDisentangledVQ(d_model, num_content_buckets)

        # --- ABLATION: Graph Processing ---
        self.relation_extractor = RelationExtractor(d_model, num_relations)
        
        if self.config["use_graph_processor"]:
            self.graph_processor = SemanticGraphProcessor(d_model, num_relations, num_layers=2)
        else:
            self.graph_processor = ProxyGraphProcessor()
        
        # --- ABLATION: Predictive Coding ---
        if self.config["use_predictive_coding"]:
            ...
            self.predictive_layers = nn.ModuleList([
                ProxyPredictiveLayer() for _ in range(n_layers // 2)
            ])
    ...
    def forward(self, x):
        ...
        # 5. Losses (Conditional)
        contrastive_loss = torch.tensor(0.0, device=x.device)
        if self.config["use_contrastive_loss"] and S > 1:
            mid = S // 2
        ...
        if self.training and self.config["use_symbolic_rules"]:
    ...
    while step < steps:
        ...
        for x, y in batches:
            try:
                ...
                if step % 500 == 0 and model.config["use_symbolic_rules"]:
