...
# ==========================================
# ADVERSARIAL QUALITY VALIDATOR COMPONENTS
# ==========================================

class SemanticCoherenceChecker(nn.Module):
    """
    Evaluates semantic coherence of generated text using attention mechanisms.
    Analyzes local and global coherence, detects repetitive patterns.
    """
    def __init__(self, d_model, n_heads=4, max_seq_len=1024):
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        
        # Self-attention for coherence analysis
    ...
    def forward(self, x, attention_mask=None):
        ...
        Returns:
            coherence_score: (B,) coherence score per sequence [0, 1]
            repetition_score: (B,) repetition score per sequence [0, 1]
        """
        B, T, D = x.shape
        residual = x
        x_norm = self.norm(x)
        ...
        # Self-attention for coherence analysis
        qkv = self.qkv(x_norm)
        q, k, v = qkv.reshape(B, T, 3, self.n_heads, D // self.n_heads).permute(2, 0, 3, 1, 4)
        ...
        # Compute coherence score (average over sequence)
        coherence = self.coherence_scorer(out).squeeze(-1)  # (B, T)
        coherence_score = coherence.mean(dim=1)  # (B,)
        
        # Detect repetitive patterns
        repetition = self.repetition_detector(out).squeeze(-1)  # (B, T)
        repetition_score = repetition.mean(dim=1)  # (B,)
        
        return coherence_score, repetition_score

class PerplexityQualityScorer(nn.Module):
    """
    Lightweight language model that scores prediction quality via perplexity.
    Trains to predict next tokens and uses prediction confidence as quality metric.
    """
    def __init__(self, vocab_size, d_model=256, n_layers=2, n_heads=4):
        super().__init__()
        ...
        # Positional encoding
        self.pos_embeddings = nn.Parameter(torch.zeros(1, 1024, d_model))
    ...
    def forward(self, input_ids, target_logits=None):
        """
        Args:
            input_ids: (B, T) token indices
            target_logits: optional (B, T, V) logits from main network to compare
        Returns:
            perplexity_score: (B,) quality score (higher = better quality)
        ...
        # Compute quality score based on prediction confidence
        # Higher confidence = better quality
        probs = F.softmax(logits, dim=-1)
        max_probs = probs.max(dim=-1)[0]  # (B, T)
        quality_score = max_probs.mean(dim=1)  # (B,)
        
        # If target logits provided, compute KL divergence loss
        loss = None
        if target_logits is not None:
            # KL divergence between validator predictions and main network predictions
            # This helps validator learn to recognize good predictions
            validator_probs = F.log_softmax(logits, dim=-1)
            target_probs = F.softmax(target_logits, dim=-1)
            loss = F.kl_div(validator_probs, target_probs, reduction='batchmean')
        
        return quality_score, loss

class AdversarialQualityValidator(nn.Module):
    """
    Main adversarial network that validates main network predictions.
    Combines semantic coherence and perplexity scores into a quality metric.
    """
    def __init__(self, d_model, vocab_size, n_heads=4, validator_hidden_dim=256, validator_num_layers=2):
        super().__init__()
        self.d_model = d_model
        self.vocab_size = vocab_size
        
        # Sub-modules
        self.coherence_checker = SemanticCoherenceChecker(d_model, n_heads)
        self.perplexity_scorer = PerplexityQualityScorer(vocab_size, validator_hidden_dim, validator_num_layers, n_heads)
        
        # Score fusion layer
        self.fusion = nn.Sequential(
            nn.Linear(3, validator_hidden_dim),  # coherence, (1-repetition), perplexity
            nn.GELU(),
            nn.Linear(validator_hidden_dim, validator_hidden_dim // 2),
            nn.GELU(),
            nn.Linear(validator_hidden_dim // 2, 1),
            nn.Sigmoid()
        )
        
        # Quality threshold (learnable)
        self.quality_threshold = nn.Parameter(torch.tensor(0.5))
        
        # EMA for stable training
        self.register_buffer('ema_quality', torch.tensor(0.5))
        self.ema_decay = 0.99
        
    def forward(self, embeddings, input_ids, main_network_logits=None, update_ema=True):
        """
        Args:
            embeddings: (B, T, D) token embeddings from main network
            input_ids: (B, T) token indices
            main_network_logits: optional (B, T, V) logits from main network
            update_ema: whether to update EMA quality estimate
        Returns:
            quality_score: (B,) overall quality score [0, 1]
            details: dict with individual component scores
            validator_loss: optional loss if main_network_logits provided
        """
        B, T, D = embeddings.shape
        
        # Get coherence scores
        coherence_score, repetition_score = self.coherence_checker(embeddings)
        
        # Get perplexity quality score
        perplexity_score, validator_loss = self.perplexity_scorer(input_ids, main_network_logits)
        
        # Combine scores
        # Higher coherence = better, lower repetition = better, higher perplexity_score = better
        combined_input = torch.stack([
            coherence_score,
            1.0 - repetition_score,  # Invert repetition (lower is better)
            perplexity_score
        ], dim=-1)  # (B, 3)
        
        # Fuse scores into single quality metric
        quality_score = self.fusion(combined_input).squeeze(-1)  # (B,)
        
        # Update EMA
        if update_ema and self.training:
            with torch.no_grad():
                self.ema_quality = self.ema_decay * self.ema_quality + (1 - self.ema_decay) * quality_score.mean()
        
        # Details for logging
        details = {
            'coherence': coherence_score.mean().item(),
            'repetition': repetition_score.mean().item(),
            'perplexity_quality': perplexity_score.mean().item(),
            'quality_score': quality_score.mean().item(),
            'ema_quality': self.ema_quality.item()
        }
        
        return quality_score, details, validator_loss
    
    def get_penalty(self, quality_score):
        """
        Compute adversarial penalty based on quality score.
        Returns penalty for each sample in batch.
        """
        # Penalty is higher when quality is below threshold
        penalty = F.relu(self.quality_threshold - quality_score)
        return penalty
...
# Model Architecture
n_layer = 6          # More layers for spectral
n_head = 6
n_embd = 384         
dropout = 0.2
label_smoothing = 0.1
...
# ==========================================
# ADVERSARIAL VALIDATOR SETTINGS
# ==========================================
adversarial_check_interval = 10  # Check every N steps
adversarial_penalty_weight = 0.5  # Weight of adversarial penalty
validator_hidden_dim = 256  # Hidden dimension for validator
validator_num_layers = 2  # Number of layers in validator LM
coherence_attention_heads = 4  # Heads for coherence checker
quality_threshold_init = 0.5  # Initial quality threshold
...
# ==========================================
# Enhanced Hierarchical Transformer with Adversarial Validator
# ==========================================
class EnhancedHierarchicalTransformer(nn.Module):
    def __init__(self, tokens_to_predict=5, vocab_size=50304):
        ...
        # Shared output head (tied with embeddings)
        self.shared_head = nn.Linear(n_embd, vocab_size, bias=False)
        
        # BERT head with shared final layer
        self.bert_head = nn.Sequential(
            nn.Linear(n_embd, n_embd),
            nn.GELU(),
            nn.LayerNorm(n_embd),
            self.shared_head  # Share final layer
        )
        
        # Weight tying: tie all output heads to embeddings
        self.tok_embeddings.weight = nn.Parameter(
            torch.cat([self.shared_head.weight, torch.randn(1, n_embd) * 0.02], dim=0)
        )
        ...
        self.adversarial_validator = AdversarialQualityValidator(
            d_model=n_embd,
            vocab_size=vocab_size,
            n_heads=coherence_attention_heads,
            validator_hidden_dim=validator_hidden_dim,
            validator_num_layers=validator_num_layers
        )
        
        # Initialize quality threshold
        self.adversarial_validator.quality_threshold.data.fill_(quality_threshold_init)
        
        # Print architecture info
        print(f"\n{'='*80}")
        print(f"Enhanced Hierarchical Transformer with Adversarial Quality Validator")
        print(f"{'='*80}")
        ...
        print(f"\n  Output Heads: ALL TIED to input embeddings (parameter sharing)")
        print(f"  Loss Balancing: Adaptive Uncertainty Weighting (Learnable)")
        print(f"  Long-Range Mechanisms: Spectral + Memory + Sliding Window")
        print(f"\n  *** ADVERSARIAL QUALITY VALIDATOR ***")
        print(f"  - Check interval: Every {adversarial_check_interval} steps")
        print(f"  - Penalty weight: {adversarial_penalty_weight}")
        print(f"  - Validator dim: {validator_hidden_dim}")
        print(f"  - Coherence heads: {coherence_attention_heads}")
        print(f"  - Quality threshold: {quality_threshold_init}")
        print(f"{'='*80}\n")

    def forward(self, idx, targets=None, bert_mode=False, bert_labels=None, 
                run_validator=False, validator_update_ema=True):
        B, T = idx.shape
        ...
        if bert_mode:
            ...
            return None, None, None, loss, None, None
            ...
            # Store embeddings for validator
            embeddings = x
            
            # All output heads now share the same weights (tied to embeddings)
            logits_main = self.shared_head(x)
            logits_spec = self.shared_head(x)
            logits_memory = self.shared_head(x)

            loss = None
            adversarial_penalty = None
            ...
            if targets is not None:
                ...
                # *** RUN ADVERSARIAL VALIDATOR IF REQUESTED ***
                if run_validator:
                    quality_score, details, validator_loss = self.adversarial_validator(
                        embeddings=embeddings,
                        input_ids=idx,
                        main_network_logits=logits_main,
                        update_ema=validator_update_ema
                    )
                    
                    # Compute adversarial penalty
                    penalty = self.adversarial_validator.get_penalty(quality_score)
                    adversarial_penalty = penalty.mean() * adversarial_penalty_weight
                    
                    # Add penalty to main loss
                    loss = loss + adversarial_penalty
                    
                    validator_details = details
                else:
                    validator_details = None

            return logits_main, logits_spec, logits_memory, loss, adversarial_penalty, validator_details

    @torch.no_grad()
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, top_p=None, repetition_penalty=1.5):
        for _ in range(max_new_tokens):
            idx_cond = idx if idx.size(1) <= block_size else idx[:, -block_size:]
            logits_main, _, _, _, _, _ = self(idx_cond, bert_mode=False)
...
def estimate_loss(model):
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            _, _, _, loss, _, _ = model(X, Y, bert_mode=False)
            losses[k] = loss.item()
...
if __name__ == '__main__':
    ...
    # Separate optimizers for main network and validator
    main_params = [p for n, p in generator.named_parameters() if 'adversarial_validator' not in n]
    validator_params = [p for n, p in generator.named_parameters() if 'adversarial_validator' in n]
    ...
    validator_optimizer = torch.optim.AdamW(
        validator_params,
        lr=learning_rate * 0.5,  # Lower LR for validator
        betas=(beta1, beta2),
    ...
    if os.path.exists(ckpt_path):
        ...
        if choice == 'c':
            ...
            if 'validator_optimizer' in checkpoint:
                validator_optimizer.load_state_dict(checkpoint['validator_optimizer'])
    ...
    validator_params_count = sum(p.numel() for p in generator.adversarial_validator.parameters())/1e6
    print(f"Generator params: {gen_params:.2f}M")
    print(f"Validator params: {validator_params_count:.2f}M")
    print(f"Total params: {gen_params + validator_params_count:.2f}M\n")
    ...
    adversarial_penalties_applied = 0
    ...
    while iter_num <= max_iters:
        ...
        if iter_num % eval_interval == 0:
            ...
            # --- Inside the training loop (when validation improves) ---
            if losses['val'] < best_val_loss or always_save_checkpoint:
                ...
                if iter_num > 0:
                    checkpoint_data = {
                        ...
                        'validator_optimizer': validator_optimizer.state_dict(),
                        'iter_num': iter_num,
                        'best_val_loss': best_val_loss,
                    }
                    torch.save(checkpoint_data, ckpt_path)
                    print(f">>> Checkpoint saved at iter {iter_num} (Loss: {best_val_loss:.4f})")

            # --- At the very end of the script (final save) ---
        ...
        for param_group in optimizer.param_groups: 
            param_group['lr'] = lr
        for param_group in validator_optimizer.param_groups:
            param_group['lr'] = lr * 0.5
        
        xb, yb = get_batch('train', mask_for_bert=False)
        is_bert_step = (bert_every_n_steps > 0) and (iter_num % bert_every_n_steps == 0)
        is_adversarial_step = (adversarial_check_interval > 0) and (iter_num % adversarial_check_interval == 0)
        
        if is_bert_step:
            with torch.amp.autocast(device_type="cuda" if "cuda" in device else "cpu", dtype=ptdtype):
                _, _, _, loss_auto, _, _ = generator(xb, yb, bert_mode=False)
                xb_masked, bert_labels, _ = get_batch('train', mask_for_bert=True)
                _, _, _, loss_bert, _, _ = generator(xb_masked, bert_labels=bert_labels, bert_mode=True)
                
                # Run adversarial validator if it's an adversarial step
                if is_adversarial_step:
                    adversarial_checks += 1
                    _, _, _, loss_auto_with_penalty, adv_penalty, adv_details = generator(
                        xb, yb, bert_mode=False, run_validator=True
                    )
                    if adv_penalty is not None and adv_penalty.item() > 0:
                        adversarial_penalties_applied += 1
                    total_loss = loss_auto_with_penalty + loss_bert
                else:
                    total_loss = loss_auto + loss_bert
            
            optimizer.zero_grad(set_to_none=True)
            validator_optimizer.zero_grad(set_to_none=True)
            total_loss.backward()
            grad_norm = compute_grad_norm(generator)
            torch.nn.utils.clip_grad_norm_(generator.parameters(), grad_clip)
            optimizer.step()
            validator_optimizer.step()
            ...
            if iter_num % log_interval == 0:
                ...
                if is_adversarial_step and adv_details:
                    ...
                    print(f"  *** ADVERSARIAL CHECK #{adversarial_checks} ***")
                    print(f"  Quality: {adv_details['quality_score']:.4f} | Coherence: {adv_details['coherence']:.4f} | "
                          f"Repetition: {adv_details['repetition']:.4f} | Perplexity: {adv_details['perplexity_quality']:.4f}")
                    if adv_penalty is not None:
                        print(f"  Penalty: {adv_penalty.item():.4f} | Penalties applied: {adversarial_penalties_applied}/{adversarial_checks}")
                else:
                    print(f"iter {iter_num}: loss_AUTO {loss_auto.item():.4f} + loss_BERT {loss_bert.item():.4f} | "
                          f"weights A:{weights['w_auto']:.3f} S:{weights['w_spectral']:.3f} "
                          f"M:{weights['w_memory']:.3f} | grad_norm {grad_norm:.4f} | time {dt*1000:.2f}ms")
            ...
            with torch.amp.autocast(device_type="cuda" if "cuda" in device else "cpu", dtype=ptdtype):
                if is_adversarial_step:
                    adversarial_checks += 1
                    _, _, _, loss_auto, adv_penalty, adv_details = generator(
                        xb, yb, bert_mode=False, run_validator=True
                    )
                    if adv_penalty is not None and adv_penalty.item() > 0:
                        adversarial_penalties_applied += 1
