...
class ParquetDataLoader:
    ...
    def load_file_in_chunks(self, file_path):
        try:
            ...
            # total_rows = parquet_file.metadata.num_rows # Unused but good for debug
            ...
            for batch in parquet_file.iter_batches(batch_size=self.chunk_size):
                ...
                # Yield if we have a substantial chunk to prevent memory spikes
...
class SpectralStateSpaceBlock(nn.Module):
    def __init__(self, d_model, n_frequencies, n_heads=4):
        ...
        # --- 1. THE VALVES (Gating Mechanisms) ---
        # Input Gate: Decides what enters the "Echo Chamber"
        self.input_gate = nn.Linear(d_model, d_model)
        
        # Output Gate: Decides if we are recalling memory right now
        self.output_gate = nn.Linear(d_model, d_model)

        # --- 2. THE RESONANCE (Spectral Parameters) ---
        # Matrix B (Input Projection)
        self.to_spectral = nn.Linear(d_model, n_frequencies * 2, bias=False)
        
        # Matrix A (Recurrence / Decay)
        # UPDATED: Initialization pushed closer to 0 (Unitary) to allow "Superconductivity"
        # -0.001 decay is extremely slow, effectively infinite memory for the context window
    ...
    def fft_convolution(self, inputs_real, inputs_imag, decay, omega):
        """
        S3M Implementation via FFT Convolution.
        """
        ...
        # Efficient Kernel Construction via Cumprod
    ...
    def forward(self, x, prev_layer_output=None):
        ...
        # --- STEP 1: INPUT GATING (Selection) ---
        # "Is this information important?"
        # If gate is near 0, we input 'silence', preserving previous echoes in the convolution
        ...
        # --- STEP 2: OUTPUT GATING (Focus) ---
        # "Do I need to recall the memory right now?"
...
def train_on_parquet_files(model, optimizer, scheduler, data_loader, device, vocab_size, 
    ...
    for epoch, (file_path, chunk_generator) in enumerate(data_loader.iterate_files(), 1):
        ...
        for raw_chunk in chunk_generator:
            ...
            if max_batches < 1: max_batches = 1 # Force at least one pass if small
